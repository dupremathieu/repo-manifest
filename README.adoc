...
You can checkout several Yocto version:

- kirkstone
- scarthgap
- wrynose

To checkout a version, point to the appropriate xml file:

Example with wrynose
```
repo init -u "https://github.com/seapath/repo-manifest.git" -m wrynose.xml
repo sync
```

It is also possible to fetch a version with frozen revision:
For instance, to checkout the v2.0.0 version:

```
repo init -u "https://github.com/seapath/repo-manifest.git" -m v2.0.0.xml
repo sync
```
